...
using Voxpop.Profile.Infrastructure.Persistence.Seed.Dtos;

namespace Voxpop.Profile.Infrastructure.Persistence.Seed;

public class Seeder(IServiceScopeFactory scopeFactory, ILogger<Seeder> logger)
{
    ...
    private async Task SeedAsync<TModel>(
        ...
        CancellationToken ct = default)
        where TModel : BaseCodeModel => await SeedAsync<TModel, BaseCodeSeed>(fileName, map, ct);
}